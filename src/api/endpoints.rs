...
use super::serialization::BodySerializer;

/// Клиент для вызова KuCoin API эндпоинтов
...
impl KuCoinEndpoints {
    ...
    async fn make_request(
        ...
        if authenticated {
            let auth_headers = self.auth.build_auth_headers(
                &method,
                endpoint,
                query_string,
                body_str,
        ...
        let response = self.client.send_request(method, &url, headers, body).await?;
